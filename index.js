...
// =============================================
// PDF PROXY ENDPOINT - Serves PDF directly (no redirect)
// =============================================
app.get("/api/pdf-proxy/:bookId", async (req, res) => {
  try {
    ...
    // For Cloudinary URLs, we need to fetch and pipe the PDF
    if (pdfUrl.includes('cloudinary.com')) {
      try {
        // Use native fetch (Node.js 18+) to get the PDF
        ...
        // Fallback: Redirect to mobile-friendly URL
      ...
      // For non-Cloudinary URLs, redirect
      res.redirect(pdfUrl);
    }
...
// =============================================
// MOBILE-FRIENDLY PDF REDIRECT
// =============================================
app.get("/api/mobile-pdf/:bookId", async (req, res) => {
  try {
    const Book = (await import("./models/Book.js")).default;
    const book = await Book.findById(req.params.bookId);
...
// =============================================
// DEBUG ENDPOINT - Test PDF URLs
// =============================================
app.get("/api/test-pdf-url/:bookId", async (req, res) => {
  try {
    const Book = (await import("./models/Book.js")).default;
    const book = await Book.findById(req.params.bookId);
    ...
    res.json({
      ...
      mobileFriendlyUrl: pdfUrl ? `${pdfUrl}?fl_attachment=false#toolbar=0&navpanes=0&scrollbar=0&view=FitH` : null,
      proxyUrl: `/api/pdf-proxy/${book._id}`,
      mobileRedirectUrl: `/api/mobile-pdf/${book._id}`
    });
