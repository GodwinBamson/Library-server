...
app.get("/api/socket-status", (req, res) => {
  ...
  res.json({
    ...
    availableStaff: stats.availableStaff || 0,
...
app.get("/api/ws-health", (req, res) => {
  ...
  res.json({
    ...
    transports: ["websocket", "polling"],
    upgradeInProgress: connectedClients > 0 ? "Active" : "No clients",
...
app.get("/api/socket-debug", (req, res) => {
  res.json({
    socketIOActive: !!io,
    allowedOrigins: io?.opts?.cors?.origin || "Not configured",
    activeUsers: io?.getActiveUsers?.()?.length || 0,
    activeRooms: io?.getActiveRooms?.()?.length || 0,
    availableStaff: io?.getAvailableStaff?.()?.length || 0,
    connectedClients: io?.engine?.clientsCount || 0,
    transport: "websocket and polling",
    stats: io?.getStats ? io.getStats() : null,
  });
});
...
httpServer.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(`   - Active users: ${io?.getActiveUsers?.()?.length || 0}`);
  console.log(`   - Active rooms: ${io?.getActiveRooms?.()?.length || 0}`);
  console.log(
    `   - Available staff: ${io?.getAvailableStaff?.()?.length || 0}`,
  );
  console.log(`   - Transports: websocket, polling`);

  if (process.env.NODE_ENV === "development") {
    console.log(
      `\n📁 Upload directory: ${path.join(__dirname, "uploads/pdfs")}`,
    );
    console.log(`\n🔍 Debug endpoints:`);
    console.log(
      `   - Socket status: http://localhost:${PORT}/api/socket-status`,
    );
    console.log(`   - WS health: http://localhost:${PORT}/api/ws-health`);
