...
const getPdfUrl = (book) => {
  ...
  // In production, if we have a path but not a full URL, construct it
  if (process.env.NODE_ENV === "production" && book.pdfFile) {
    const filename = book.pdfFile.split('/').pop();
    return `https://res.cloudinary.com/${process.env.CLOUDINARY_CLOUD_NAME}/raw/upload/library-books/${filename}`;
  }
...
export const createBook = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      console.log("   - Is full URL?", req.file.path.startsWith('http'));
    }
    ...
    if (req.file) {
      if (process.env.NODE_ENV === "production") {
        // Use the full URL that was constructed in the middleware
        bookData.pdfFile = req.file.path;
        bookData.pdfFilename = req.file.originalname;
        console.log("✅ PDF saved with FULL URL:", bookData.pdfFile);
      } else {
        // Store local filename only
        bookData.pdfFile = req.file.filename;
        bookData.pdfFilename = req.file.originalname;
...
export const updateBook = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      // Store new PDF
      if (process.env.NODE_ENV === "production") {
        updateData.pdfFile = req.file.path; // Full Cloudinary URL
        updateData.pdfFilename = req.file.originalname;
        console.log("✅ New PDF saved with FULL URL:", updateData.pdfFile);
      } else {
        updateData.pdfFile = req.file.filename;
        updateData.pdfFilename = req.file.originalname;
        console.log("✅ New PDF saved locally:", updateData.pdfFile);
      }
    }
