...
//     // For production with Cloudinary - just redirect to the public URL
//     if (process.env.NODE_ENV === "production" && book.pdfFile.includes("cloudinary.com")) {
//       // Cloudinary URLs are publicly accessible, so we can redirect
//       const cloudinaryUrl = book.pdfFile.replace(/\/v\d+\//, "/");
...
//     // For development or local files - we need auth check
//     const authHeader = req.headers.authorization;
//     if (!authHeader || !authHeader.startsWith("Bearer ")) {
//       // For development, we can still serve without strict auth if needed
//       if (process.env.NODE_ENV === "development") {
//         console.log(" Development mode: serving without strict auth");
//       } else {
//         return res.status(401).json({ message: "Not authorized" });
//       }
//     }
