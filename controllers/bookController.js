...
export const servePdf = async (req, res) => {
  try {
    ...
    // Important: Set CORS headers for production
    ...
    if (process.env.NODE_ENV === "production") {
      // For production, if it's a Cloudinary URL, redirect to it directly
      if (book.pdfFile.includes("cloudinary.com")) {
        const cloudinaryUrl = book.pdfFile.replace(/\/v\d+\//, "/");
      ...
      // If not Cloudinary (shouldn't happen in production), serve through backend
      // This shouldn't happen since production should use Cloudinary
      return res.status(404).json({ message: "PDF not available in production mode" });
    } else {
      // Development: serve local file
      const pdfPath = path.join(
        __dirname,
