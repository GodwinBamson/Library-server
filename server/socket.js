...
export const setupSocketIO = (server) => {
    ...
    cors: {
      origin:
        process.env.NODE_ENV === "production"
          ? [
              "https://frontend-libraryapp.onrender.com", // Frontend URL
              "https://library-server-5rpq.onrender.com", // Backend URL (for self)
            ]
          : [
              "http://localhost:3000",
              "http://localhost:5173",
  ...
  io.on("connection", (socket) => {
    ...
    console.log(
      `🔵 User connected: ${username} (${userId}) - Role: ${role} - Socket: ${socket.id}`,
    );
    ...
    socket.on("request-staff-list", () => {
      ...
      console.log(
        `📋 Sending staff list to ${username}: ${staffList.length} staff available`,
      );
    ...
    socket.on("direct-message", ({ to, message }) => {
      ...
      if (targetSocketId) {
        ...
        console.log(
          `💬 Direct message from ${username} to ${to} (offline - stored)`,
        );
    ...
    socket.on("join-room", ({ roomId, callType }) => {
      try {
        ...
        console.log(
          `👥 ${username} joining room: ${normalizedRoomId} (${callType} call)`,
        );
            ...
            for (const [uid, sid] of userSockets.entries()) {
              if (sid === id) {
                return {
                  ...
                  userId: uid,
                  username:
                    io.sockets.sockets.get(id)?.userData?.username || "Unknown",
    ...
    socket.on("send-message", (message) => {
      console.log(
        `💬 Message in ${message.roomId} from ${username}: ${message.text?.substring(0, 30)}`,
      );
    ...
    socket.on(
      "message-reaction",
      ({ messageId, reaction, userId: reactingUserId }) => {
        console.log(
          `👍 Reaction ${reaction} on message ${messageId} from ${reactingUserId}`,
        );
    ...
    socket.on("disconnect", (reason) => {
      ...
      console.log(
        `   Active users: ${userSockets.size}, Active rooms: ${rooms.size}`,
      );
    });
